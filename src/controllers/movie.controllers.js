...
const setMovieGenres = catchError(async(req, res) => {
    ...
    if (!movie) return res.status(404).json({message: "The gender in movie not found"});
...
const setMovieActors = catchError(async(req, res) => {
    const { id } = req.params;
    const movie = await Movie.findByPk(id);
    if (!movie) return res.status(404).json({message: "The actor in movie not found"});
...
const setMovieDirectors = catchError(async(req, res) => {
    const { id } = req.params;
    const movie = await Movie.findByPk(id);
    if (!movie) return res.status(404).json({message: "The director in movie not found"});
